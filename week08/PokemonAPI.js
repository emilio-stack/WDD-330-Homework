...
function displayBox(pokedex,x) {
  ...
  for (let i=0; i < pokedex[x].length; i++) {
    ...
    let detailsButton = document.createElement('button');                               // Create a button to display details
    detailsButton.id = pokedex[x][i].name                                               // Add an id
    ...
    pokeCard.appendChild(detailsButton);                                                // Add button to card
...
pokdex = getFullPokemon();
console.log(pokedex)

/* The code to display the details */

function toggleDisplayDetails(pokemon) {
...
let temp  = document.getElementById('details-button').addEventListener('click', toggleDisplayDetails(pokedex));
